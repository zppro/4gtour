/**
 * recharge Created by zppro on 17-3-2.
 * Target:老人个人账户充值 (移植自fsrok)
 */
...
(function() {
    ...
    angular
        ...
        .controller('RechargeGridController', RechargeGridController)
        .controller('RechargeDetailsController', RechargeDetailsController)
    ;


    RechargeGridController.$inject = ['$scope', 'ngDialog', 'vmh', 'entryVM'];

    function RechargeGridController($scope, ngDialog, vmh, vm) {
        ...
        function booking(row) {
            ngDialog.openConfirm({
                ...
                vmh.psnService.bookingRecharge(row._id, {
    ...
    RechargeDetailsController.$inject = ['$scope', 'ngDialog', 'vmh', 'entityVM'];

    function RechargeDetailsController($scope, ngDialog, vmh, vm) {
        ...
        function init() {
            ...
            vm.load().then(function() {
                ...
                if(vm.model._id) {
                    vmh.psnService.checkCanChangeBookingOrUnbookingRecharge(vm.model._id).then(function (ret) {
                        console.log(ret);
        ...
        function disableRechargeAndUnbooking(){
            if(vm.model._id){
                ngDialog.openConfirm({
                    ...
                    vmh.psnService.disableRechargeAndUnbooking(vm.model._id,{
                        operated_by: vm.operated_by,
                        operated_by_name: vm.operated_by_name
                    }).then(function (ret) {
            ...
            if ($scope.theForm.$valid) {
                vm.save(true).then(function(ret){
                    if(!vm.model.voucher_no){
                        return vmh.psnService.bookingRecharge(vm.model._id || ret._id, {
                            operated_by: vm.operated_by,
                            operated_by_name: vm.operated_by_name
                        }).then(function () {
                            vmh.alertSuccess('notification.BOOKING-SUCCESS',true);
                        ...
                        if (vm.rawRechargeAmount != vm.model.amount) {
                            vmh.psnService.changeRechargeBookingAmount(vm.model._id,{
                                operated_by: vm.operated_by,
                                operated_by_name: vm.operated_by_name
                            }).then(function (ret) {
